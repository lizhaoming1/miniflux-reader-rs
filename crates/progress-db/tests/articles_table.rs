use progress_db::{run_migrations, ArticleRepository, ArticleUpsert, FeedRepository};
...
async fn t01_upsert_dedupes_on_guid() {
    ...
    let inserted2 = arts
        .upsert(f.id, &upsert("g1", "A — updated"))
        .await
        .unwrap();
    assert!(inserted1);
...
async fn t04_mark_all_read_single_feed() {
    ...
    let arts = ArticleRepository::new(pool.clone());
    for i in 1..=3 {
        arts.upsert(f1.id, &upsert(&format!("g-f1-{i}"), &format!("F1-{i}")))
            .await
            .unwrap();
        arts.upsert(f2.id, &upsert(&format!("g-f2-{i}"), &format!("F2-{i}")))
            .await
            .unwrap();
    }
