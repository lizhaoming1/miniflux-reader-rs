...
pub mod tts {
    ...
    impl TtsService for ReqwestTtsService {
        async fn synthesize(&self, _t: &str) -> SvcResult<Vec<u8>> {
            Err(ServiceError::Placeholder)
        }
        async fn highlights(&self, _t: &str) -> SvcResult<Vec<HighlightToken>> {
            Err(ServiceError::Placeholder)
        }
    }
}
